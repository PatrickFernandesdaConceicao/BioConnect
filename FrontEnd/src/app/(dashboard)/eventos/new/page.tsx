...
import { ArrowLeft, Save, Calendar, MapPin, User, Plus, Trash2 } from "lucide-react";
...
const eventoSchema = z.object({
  ...
  local: z.string().min(3, { message: "O local deve ter pelo menos 3 caracteres" }),
  justificativa: z.string().min(20, { message: "A justificativa deve ter pelo menos 20 caracteres" }),
  vlTotalSolicitado: z.string().min(1).refine(val => parseFloat(val) >= 0, { message: "O valor não pode ser negativo" }),
  vlTotalAprovado: z.string().min(1).refine(val => parseFloat(val) >= 0, { message: "O valor não pode ser negativo" }),
...
export default function NewEventoPage() {
  ...
  async function onSubmit(values: EventoFormValues) {
    ...
    try {
      ...
      console.error(error);
      toast.error("Erro ao cadastrar evento. Verifique os dados e tente novamente.");
    } finally {
      setIsSubmitting(false);
    }
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Preencha os dados do evento institucional.</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 mt-4">
                <TabsContent value="info" className="space-y-4">
                  <FormField control={form.control} name="titulo" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Ex: Simpósio de Robótica" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <FormField control={form.control} name="dataTermino" render={({ field }) => (
                      <FormItem>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <FormField control={form.control} name="vlTotalSolicitado" render={({ field }) => (
                      <FormItem>
                    ...
                    <FormField control={form.control} name="vlTotalAprovado" render={({ field }) => (
                      <FormItem>
                ...
                <TabsContent value="participants" className="space-y-4">
                  <div className="grid grid-cols-2 gap-2">
                    <Input placeholder="Nome" value={novoParticipante.nome} onChange={(e) => setNovoParticipante({ ...novoParticipante, nome: e.target.value })} />
                    <Input placeholder="Email" value={novoParticipante.email} onChange={(e) => setNovoParticipante({ ...novoParticipante, email: e.target.value })} />
                  </div>
                  <Button type="button" onClick={adicionarParticipante} className="mt-2">
                  ...
                  <ul className="mt-4 space-y-2">
                    {participantes.map((p, idx) => (
                      <li key={idx} className="flex items-center justify-between border p-2 rounded">
                        ...
                        <Button variant="destructive" size="icon" onClick={() => removerParticipante(idx)}>
